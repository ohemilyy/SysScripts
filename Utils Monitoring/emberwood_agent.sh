...
HTProcesses=$(pgrep -f emberwood_agent.sh | wc -l)
if [ -z "$HTProcesses" ]
then
	HTProcesses=0
fi
...
then
	pgrep -f emberwood_agent.sh | xargs kill -9
fi
for PID in $(pgrep -f emberwood_agent.sh)
do
...
then
	M=0
	# Clear the emberwood_cron.log every hour
	rm -f "$ScriptPath"/emberwood_cron.log
fi
...
# Identify system based on IP address and hostname
ID=$(echo -ne "$(hostname -f)|$(hostname -i)" | base64)
POST="v=$VERSION&s=$SID&d=$DATA&i=$ID"
# Save data to file
echo "$POST" > "$ScriptPath"/emberwood_agent.log

# Post data
POSTURL="https://api.hydride.dev/api/v1/metrics"
wget --retry-connrefused --waitretry=1 -t 3 -T 15 -qO- --post-file="$ScriptPath/emberwood_agent.log" $POSTURL &> /dev/null
